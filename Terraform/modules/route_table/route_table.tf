resource "oci_core_route_table" "route_table" {
  ...
  dynamic "route_rules" {
    ...
    content {
      ...
      network_entity_id = each.value.gateway_type == "internet" ? try(data.oci_core_internet_gateways.existing_internet_gateways[each.key].id, null) : try(data.oci_core_service_gateways.existing_service_gateways[each.key].id, null)
